...
class UserServices {
  private signAccessToken({ user_id, verify }: { user_id: string; verify: UserVerifyStatus }) {
    return signToken({
      ...
      options: {
        expiresIn: envConfig.accessExpiresIn
      }
    })
  }
  ...
  private signRefeshToken({ user_id, verify, exp }: { user_id: string; verify: UserVerifyStatus; exp?: number }) {
    ...
    return signToken({
      ...
      privateKey: envConfig.jwtSecretRefreshToken,
      options: {
        expiresIn: envConfig.refreshExpiresIn
      }
    })
  }
  ...
  private signEmailVerifyToken({ user_id, verify }: { user_id: string; verify: UserVerifyStatus }) {
    return signToken({
      ...
      options: {
        expiresIn: envConfig.emailVerifyExpiresIn
      }
    })
  }
  ...
  async register(payload: RegisterReqBody) {
    ...
    /*Flow verify email
      ...
      4. Server verify emai_verify_token
  ...
  async forgotPassword({ user_id, verify, email }: { user_id: string; verify: UserVerifyStatus; email: string }) {
    ...
    // Send email with link to user email: https/twitter.com/forgot-password?token=token
    await sendForgotPasswordTemplate(email, forgot_password_token)
    return {
